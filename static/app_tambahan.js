...
async function renderRiwayatAnggota() {
  ...
  main.innerHTML =
    '<button class="btn-sm outline" onclick="state.activeRiwayatRek=null;renderRiwayatAnggota();" style="margin-bottom:12px;">← Cari Lain</button>'+
...
async function renderJadwalNotif() {
  ...
  const tipeLabel = {"reminder_h3":"⏰ Reminder H-3 Jatuh Tempo","laporan_harian":"📊 Laporan Harian","rekap_mingguan":"📋 Rekap Mingguan (Senin)"};
  const jadwalHtml = jadwal.map(j=>
    '<div class="card admin-section" style="margin-bottom:12px;">'+
    '<div style="font-size:13px;font-weight:800;margin-bottom:8px;">'+(tipeLabel[j.tipe]||j.tipe)+'</div>'+
    '<div style="font-size:12px;color:var(--gray-500);margin-bottom:12px;">'+j.keterangan+'</div>'+
    '<div style="display:flex;gap:10px;align-items:center;margin-bottom:10px;">'+
      '<div style="flex:1;"><div class="modal-label">Jam Kirim</div>'+
      '<input type="time" id="jam_'+j.id+'" value="'+j.jam+'" class="modal-input" style="margin-bottom:0;"/></div>'+
      '<div><div class="modal-label">Status</div>'+
      '<select id="aktif_'+j.id+'" class="modal-input" style="margin-bottom:0;">'+
        '<option value="1" '+(j.aktif?"selected":"")+'>✅ Aktif</option>'+
        '<option value="0" '+(!j.aktif?"selected":"")+'>⏸️ Nonaktif</option>'+
      '</select></div></div>'+
    '<div style="display:flex;gap:8px;">'+
      '<button class="btn-sm green" onclick="simpanJadwal('+j.id+')">💾 Simpan</button>'+
      '<button class="btn-sm outline" onclick="testNotif(\''+j.tipe+'\')">🧪 Test Kirim</button>'+
    '</div>'+
    '<div id="jadwal_result_'+j.id+'" style="margin-top:8px;"></div></div>'
  ).join("");
  main.innerHTML =
    '<div class="section-title">🔔 Pengaturan Notifikasi</div>'+
    '<div style="background:var(--yellow-pale);border-radius:8px;padding:12px;font-size:12px;color:var(--yellow-dark);margin-bottom:12px;">'+
    '⚠️ Notifikasi dikirim otomatis sesuai jadwal selama app aktif di browser.</div>'+
    jadwalHtml+
    '<div class="section-title">📲 Kirim Manual Sekarang</div>'+
    '<div class="card admin-section"><div style="display:flex;flex-direction:column;gap:10px;">'+
    '<button class="btn-primary" onclick="testNotif(\'reminder_h3\')">⏰ Kirim Reminder H-3</button>'+
    '<button class="btn-primary" style="background:var(--green-mid);" onclick="testNotif(\'laporan_harian\')">📊 Kirim Laporan Harian</button>'+
    '<button class="btn-primary" style="background:var(--yellow-dark);" onclick="testNotif(\'rekap_mingguan\')">📋 Kirim Rekap Mingguan</button>'+
    '</div><div id="testNotifResult" style="margin-top:12px;"></div></div>';
}
async function simpanJadwal(id) {
  const jam = document.getElementById("jam_"+id).value;
  const aktif = document.getElementById("aktif_"+id).value;
  const res = await api("/api/jadwal-notif/"+id, "PUT", {jam, aktif});
  const el = document.getElementById("jadwal_result_"+id);
  if (res.success) {
    el.innerHTML='<div style="color:var(--green-dark);font-size:12px;font-weight:700;">✅ Jadwal disimpan!</div>';
...
async function testNotif(tipe) {
  ...
  if (el) {
    if (res.success) {
      el.innerHTML='<div style="background:var(--green-pale);border-radius:8px;padding:10px;font-size:12px;">'+
        '✅ Berhasil · Terkirim: <strong>'+(res.terkirim||0)+'</strong> · Gagal: <strong>'+(res.gagal||0)+'</strong>'+
        (res.preview_pesan?'<details style="margin-top:8px;"><summary style="cursor:pointer;font-weight:700;">Preview Pesan</summary>'+
        '<pre style="font-size:10px;white-space:pre-wrap;margin-top:8px;background:#f5f5f5;padding:8px;border-radius:4px;">'+res.preview_pesan+'</pre></details>':'')+'</div>';
    } else {
      el.innerHTML='<div class="error-msg">❌ '+(res.error||"Gagal")+'</div>';
    }
  }
